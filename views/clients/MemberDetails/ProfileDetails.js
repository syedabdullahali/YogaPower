...
const ProfileDetails = ({ ids, deleteId }) => {
    ...
    function getDetails(id) {
        ...
        axios.get(`${url}/memberForm/${id}`, { headers },
            ...
            .then((resp) => {
                ...
                setWhatsappNumber(resp.data.ContactNumber)
                setGender(resp.data.Gender)
                setDateofBirth(moment(resp.data.DateofBirth).format("YYYY-MM-DD"))
                setAnniversarydate(moment(resp.data.Anniversarydate).format('YYYY-MM-DD'))
                setAddress(resp.data.Address)
                setArea(resp.data.Area)
                setCity(resp.data.city)
                setState(resp.data.state)
                setPincode(resp.data.pincode)
                setBloodGroup(resp.data.BloodGroup)
                setFacebookID(resp.data.FacebookID)
                setsms(resp.data.sms)
                setmail(resp.data.mail)
                setpushnotification(resp.data.pushnotification)
                setName(resp.data.Name)
                setRelationship(resp.data.Relationship)
                setCountryCode1(resp.data.CountryCode1)
                setContactNumber1(resp.data.ContactNumber1)
                setSer4(resp.data.ServiceVariation)
                setserviceName(resp.data.serviceName)
                setCustomertype(resp.data.Customertype)
                setEnquiryType(resp.data.EnquiryType)
                setMemberManager(resp.data.MemberManager)
                setBatch(resp.data.Batch)
                setGeneralTrainer(resp.data.GeneralTrainer)
                setAttendanceID(resp.data.AttendanceID)
                setCenterID(resp.data.CenterID)
                setLockerKeyNo(resp.data.LockerKeyNo)
                setPAN(resp.data.PAN)
                setOther(resp.data.Other)
                setAssignStaff(resp.data.AssignStaff)
                setWeight(resp.data.Weight)
                setHeight(resp.data.Height)
                setfitnessLevel(resp.data.fitnessLevel)
                setfitnessGoal(resp.data.fitnessGoal)
                setidealWeight(resp.data.idealWeight)
    ...
    const saveMember = () => {
        ...
        axios.post(`${url}/memberForm/update/${ids}`, data, { headers },
        )
            .then((resp) => {
